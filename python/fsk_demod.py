...
class fsk_demod(gr.hier_block2):
	def __init__(self, syms_per_sec, samples_per_sec, gain_mu, mu, omega_relative_limit, freq_offset):
		...
		self._downsampletaps = firdes.low_pass(1, self._samples_per_second, 10000, 1000, firdes.WIN_HANN)
		...
		self._downsample = filter.freq_xlating_fir_filter_ccc(self._decim, #decimation
		...
		self._demod = analog.pll_freqdet_cf(0.8, #gain alpha, rad/samp
		...
		self._carriertrack = analog.pll_freqdet_cf(0.3,
											   10e-6,
											   6,
											   -6)

		self._lpfcoeffs = firdes.low_pass(1000, self._samples_per_second / self._decim, self._syms_per_sec, 100, firdes.WIN_HANN)

		self._lpf = filter.fir_filter_fff(self._filtdecim, #decimation
